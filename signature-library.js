...
savePNGButton.addEventListener("click", function (event) {
  if (signaturePad.isEmpty()) {
    ...
    var pngObject = signaturePad.toDataURL();       //it is a html canvas element that converts the data in url format specified by the format specified

    if (window.opener) {
      // // Call the receiveValueFromPopup function in the parent window and pass the value
      // window.opener.receiveValueFromPopup(pngObject);
      // Send the value to the parent window using postMessage
      window.opener.postMessage(pngObject, "*");
      // Close the popup window (optional)
      window.close();
  }
    // console.log(pngObject);
    var xhr = new XMLHttpRequest();             //it is a http request we are sending post request
    xhr.open('POST', 'https://tstdrv1338970.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=2070&deploy=1&compid=TSTDRV1338970&h=dde2fdd0f9743e483eb7', true);
    xhr.setRequestHeader('Content-Type', 'image/png');           //setting the content type 
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4 && xhr.status == 200) {
        console.log(xhr.responseText);       // success callback
      }
    };
    xhr.send(pngObject);
  }
